%% Behaviour of the dominant pole by changing the weight power (iota)
clc
clear
close
...
rho   = 0.5;
alpha = 0.5;
ratio = 0.1;      %% The ratio of the divided network as defined in page 13
...
num_points = 100; %% The number of weights that you want to consider in the interval (0,1].
...
for pwrw = linspace(0.01, 1, num_points)
i = i+1;
pwrq = pwrw;  
...
if condition1 || condition2 || condition3
    disp (['For pwrw=', num2str(pwrw), ': complex pole on a unit circle exists that make the determinant zero.'])
    dominant_zp(i) = 1.;
else
    dom_zp = find_dominant(all_roots);    
    disp (['For pwrw=', num2str(pwrw), ': the dominant pole is:', num2str(double(dom_zp))])
    dominant_zp(i) = double(abs(dom_zp));
end

if condition1 == previous_condition
    condition_counter = condition_counter + 1;
else
    condition_counter = 0;  
end
previous_condition = condition1;
if condition_counter >= max_consecutive_iterations && pwrw>0.2 && condition1 == true
    disp('Condition unchanged for 5 consecutive iterations. Breaking the loop.');
    break;
end
end
...
xlabel('\bf{Power of Weights}', 'Interpreter', 'latex', 'FontSize', 26);
...
grid on;
legend_text = sprintf('$n/N = %.2f$', ratio);
legend(legend_text, 'Interpreter', 'latex', 'FontSize', 28, 'Location', 'southeast');
set(gcf, 'Color', 'w');

filename = sprintf('dom_zp_ratio_%.2f_alpha_%.2f_rho_%.2f_gamma_%.2f.png', ratio, alpha, rho, gamma);
saveas(gcf, filename);
